...
if __name__ == "__main__":
        ...
        # Figure label "B"
        ax1.text(-0.3*np.shape(ll_map_m)[0], 1.2*np.shape(ll_map_m)[0], 'C', fontsize=FS+10, fontweight='bold', color=TCOLOR)
        ax1.text(-0.15*np.shape(ll_map_m)[0], 1.2*np.shape(ll_map_m)[0], "$\\bf{In~Vivo~Data}$", fontsize=FS+2, fontweight='bold', color=TCOLOR)
        ...
        for i in range(0,len(ll_values)):
                ax4.errorbar(ll_values[i], bloch_values[i], xerr=ll_std[i], yerr=bloch_std[i], fmt='*', color=COLORS[i], label='ROI '+str(i+1))
