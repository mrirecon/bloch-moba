...
Bloch_flash)
	...
	--other pinit=1:1:1:0,pscale=1:1:1:0.1 --scale_data=5000. --scale_psf=1000. --normalize_scaling \
        -i$ITER -C$INNER_ITER -s$STEP_SIZE -B$MIN_R1 -d4 -o$OS -R$REDU_FAC -j$LAMBDA -g -N \
        -t ${TRAJ} ${DATA} TI reco sens \
        2>&1 | tee reco.log
...
Bloch_2_5 | Bloch_2_1 | Bloch_1_6 | Bloch_1_2 | Bloch_0_6 | Bloch_0_4 )
	...
	--other pinit=1:1:1:0,pscale=1:1:30:0,b1-sobolev-a=440,b1-sobolev-b=22,b1map=${B1MAP} --scale_data=5000. --scale_psf=1000. --normalize_scaling \
        -i$ITER -C$INNER_ITER -s$STEP_SIZE -B$MIN_R1 -d4 -o$OS -R$REDU_FAC -j$LAMBDA -g -N \
        --pusteps 4 --ratio 0.5 \
	-t ${TRAJ} ${DATA} TI reco sens \
        2>&1 | tee reco.log
