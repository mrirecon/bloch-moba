...
if __name__ == "__main__":
        ...
        # Figure label "A"
        ax1.text(-0.3*np.shape(ll_map_m)[0], 1.2*np.shape(ll_map_m)[0], 'B', fontsize=FS+10, fontweight='bold', color=TCOLOR)
        ax1.text(-0.15*np.shape(ll_map_m)[0], 1.2*np.shape(ll_map_m)[0], "$\\bf{Phantom~Data}$", fontsize=FS+2, fontweight='bold', color=TCOLOR)
