...
M0A)
        ...
        --img_dims $BR:$BR:1 \
        --other pinit=1:1:0:1,pscale=1:1:0.2:1 --scale_data=5000. --scale_psf=1000. --normalize_scaling \
        -i$ITER -C$INNER_ITER -s$STEP_SIZE -B$MIN_R1 -d4 -o$OS -R$REDU_FAC -j$LAMBDA -g -N \
        -t ${TRAJ} ${DATA} TI reco sens \
        2>&1 | tee reco.log
...
Bloch)
        ...
        --other pinit=1:1:1:0,pscale=1:1:1:0.1,b1-sobolev-a=440,b1-sobolev-b=22,stm-tol=${STMTOL},ode-tol=${STMTOL} --scale_data=5000. --scale_psf=1000. --normalize_scaling \
        -i$ITER -C$INNER_ITER -s$STEP_SIZE -B$MIN_R1 -d4 -o$OS -R$REDU_FAC -j$LAMBDA -g -N \
        -t ${TRAJ} ${DATA} TI reco sens \
        2>&1 | tee reco.log
...
Bloch_SP)
	...
	SLICE_THICKNESS=0.03
        ...
        nice -5 bart moba --bloch --sim STM \
        --img_dims $BR:$BR:1 \
        --seq IR-FLASH,TR=${TR},TE=${TE},FA=${FA},Trf=${RF_DUR},BWTP=${BWTP},pinv,ipl=${INV_LEN},ppl=${PREP_LEN},isp=${INV_SPOILER},av-spokes=${AV},sl-grad=${SS_GRAD_STRENGTH},slice-thickness=${SLICE_THICKNESS},Nspins=${SLICE_PROFILE_SPINS} \
        --other pinit=1:1:1:0,pscale=1:1:1:0.1,b1-sobolev-a=440,b1-sobolev-b=22,stm-tol=${STMTOL},ode-tol=${STMTOL} --scale_data=5000. --scale_psf=1000. --normalize_scaling \
        -i$ITER -C$INNER_ITER -s$STEP_SIZE -B$MIN_R1 -d4 -o$OS -R$REDU_FAC -j$LAMBDA -g -N \
        -t ${TRAJ} ${DATA} TI reco sens \
        2>&1 | tee reco.log
