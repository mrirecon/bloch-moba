...
Bloch)
	...
	--other pinit=1:1:1:0,pscale=1:1:30:0,b1map=${B1MAP},b1-sobolev-a=440,b1-sobolev-b=22,export-ksp-sens \
...
Bloch_c)
	...
	SENS=results/Bloch/sens

	if [ ! -f ${SENS}".cfl" ];
        then
                echo "Sensitivities $SENS do not exist!"
                echo "Please run Bloch model first."
                exit 1
        fi
        ...
        --img_dims $BR:$BR:1 \
        --seq IR-BSSFP,TR=${TR},TE=${TE},FA=${FA},Trf=${RF_DUR},BWTP=${BWTP},ipl=${INV_LEN},ppl=${PREP_LEN},sl-grad=${SS_GRAD_STRENGTH},isp=${INV_SPOILER},av-spokes=${AV},slice-thickness=${SLICE_THICKNESS},nom-slice-thickness=${NOM_SLICE_THICKNESS},Nspins=${SLICE_PROFILE_SPINS} \
	--other pinit=1:1:1:0,pscale=1:1:30:0,b1map=${B1MAP},b1-sobolev-a=440,b1-sobolev-b=22,ksp-sens=${SENS},no-sens-l2,no-sens-deriv \
	--scale_data=5000. --scale_psf=1000. --normalize_scaling \
        -i$ITER -C$INNER_ITER -s$STEP_SIZE -B$MIN_R1 -d4 -o$OS -R$REDU_FAC -j$LAMBDA -g -N \
	--not-wav-maps 4 \
	--l2-on-parameters 0 \
