...
if __name__ == "__main__":
	...
	for d in range(0, 3):
		...
		if (2 == d):
			...
			# ax3.set_yticklabels([])
			ax3.text(0.7*np.max(mean), +0.01, 'mean', color='black', fontsize=FS)
		...
		if (0 == d):
			removed_first_values_t2 = 1
			removed_last_values_t2 = -1
		if (1 == d):
			removed_first_values_t2 = 1
			removed_last_values_t2 = -1
		if (2 == d):
		...
		data_ref = np.asarray(reft2[removed_first_values_t2:removed_last_values_t2])
		data_meas = np.asarray(t2[ind][removed_first_values_t2:removed_last_values_t2])
		...
		for i in range(0, np.shape(data_ref)[0]):
			...
			ax4.scatter(mean[i], diff[i], marker='*', s=400, color=COLORS[i-1+removed_first_values_t2], label='ROI: '+str(i))
		...
		if (2 == d):
			# ax4.set_yticklabels([])
			ax4.text(0.7*np.max(mean), -0.005, 'mean', color='black', fontsize=FS)
